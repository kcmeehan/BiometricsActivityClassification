...
class FeatureCalc:
    ...
    def load_new_ts(self,input_df):
        ...
        #feature names
        self.feat_labels=[x+y for y in ['_mean','_median','_std','_peak','_kurtosis','_min','_max','_iqr'] for x in self.indf.columns[2:]]+['hand_acc_vsum_welch','hand_acc_vsum_sp_entropy','hand_acc_spectrum_energy','hand_acc_power_ratio']+['hand_acc_XcY','hand_acc_XcZ','hand_acc_YcZ','hand_acc_acs','hand_acc_acd']+['chest_acc_vsum_welch','chest_acc_vsum_sp_entropy','chest_acc_spectrum_energy','chest_acc_power_ratio']+['chest_acc_XcY','chest_acc_XcZ','chest_acc_YcZ','chest_acc_acs','chest_acc_acd']+['ankle_acc_vsum_welch','ankle_acc_vsum_sp_entropy','ankle_acc_spectrum_energy','ankle_acc_power_ratio']+['ankle_acc_XcY','ankle_acc_XcZ','ankle_acc_YcZ','ankle_acc_acs','ankle_acc_acd']+['hand_gyro_vsum_welch','hand_gyro_vsum_sp_entropy','hand_gyro_spectrum_energy','hand_gyro_power_ratio']+['hand_gyro_XcY','hand_gyro_XcZ','hand_gyro_YcZ','hand_gyro_acs','hand_gyro_acd']+['chest_gyro_vsum_welch','chest_gyro_vsum_sp_entropy','chest_gyro_spectrum_energy','chest_gyro_power_ratio']+['chest_gyro_XcY','chest_gyro_XcZ','chest_gyro_YcZ','chest_gyro_acs','chest_gyro_acd']+['ankle_gyro_vsum_welch','ankle_gyro_vsum_sp_entropy','ankle_gyro_spectrum_energy','ankle_gyro_power_ratio']+['ankle_gyro_XcY','ankle_gyro_XcZ','ankle_gyro_YcZ','ankle_gyro_acs','ankle_gyro_acd']+['activityID']
    ...
    def calculate_features(self):
        ...
        features = [self.mean,self.median,self.std,self.peak,self.kurtotis,self.minimum,self.maximum,self.interquartilerange]
        ...
        for df in [self.hand_acc,self.chest_acc,self.ankle_acc,self.hand_gyro,self.chest_gyro,self.ankle_gyro]:
            ...
            acs = self.autocorrelation_slope(vector_sum)
            acd = self.autocorrelation_diff(vector_sum)
            
            arr = np.concatenate((arr,np.array([welch,spectral_entropy,spectrum_energy,power_ratio,XcY,XcZ,YcZ,acs,acd])))
    ...
    def minimum(self):
        ...
        '''
        Return minimum of all the columns in the df
        '''
        
        return self.indf.min().values[2:]
    
    def maximum(self):
        
        '''
        Return maximum of all the columns in the df
        '''
        
        return self.indf.max().values[2:]
    ...
    def autocorrelation_slope(self,X):
        
        '''
        Return slope of the autocorrelation function from lags 1 to 5
        '''
        ac = acf(X,nlags=5)
        return (ac[-1]-ac[0])/5.0
        
         
    def autocorrelation_diff(self,X):
        
        '''
        Return the difference betweeen the autocorrelation function 
        at lags 1 tot 5
        '''
        
        ac = acf(X,nlags=5)
        return abs(ac[0] - ac[4])
